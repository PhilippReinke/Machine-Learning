...
class Snake:
	...
	def __play(self, player="human", fps=5, agent=0):
		...
		if "train" in player or "performance" in player:
			start = time.time()
	...
	def train_agent(self, agent, numGames, greedy=False, verbose=False):
		...
		try:
			...
			for i in range(1, numGames+1):
				...
				if greedy:
					player += " greedy"
				self.__play(player=player, fps=-1, agent=agent)
				# alle 100 Runden finale Länge der Schlange ausgeben
	...
	def evaluate_performance(self, agent_path):
		# Ausgabe nicht anzeigen
		os.environ["SDL_VIDEODRIVER"] = "dummy"
		try:
			...
			for i in range(0, 100):
				self.__play(player="agent performance", fps=-1, agent=agent)
				snake_lengths.append(len(self.snake))
		finally:
			# Ausgabemodus zurücksetzen
			os.environ["SDL_VIDEODRIVER"] = ""

		return sum(snake_lengths)/100, min(snake_lengths), max(snake_lengths)