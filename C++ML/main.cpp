...
// Prototypen
vector<vector<Matrix<double>>> load_Iris(double train_test_split);
vector<vector<Matrix<double>>> load_MNIST(double train_test_split, int num_examples);
...
int main()
{
	...
	cout << "Lineare Klassifizierung     : Performance " << res_lin_test.first / (double) res_lin_test.second << endl;

	// Polynomiale Klassifizierung
	unsigned MAX_POLY_DEGREE = 10; // >= 2
	double best_ratio_poly = -1.0; unsigned best_degree_poly;
	for (unsigned degree=2; degree<=MAX_POLY_DEGREE; degree++)
	{
		...
		double current_ratio = res_test.first / (double) res_test.second;
	...
	double best_ratio_fourier = -1.0; unsigned best_degree_fourier;
...
vector<vector<Matrix<double>>> load_Iris(double train_test_split)
{
	...
	if (file_train.is_open())
	{
		...
		while ( getline(file_train, line) )
		{
			...
			Matrix<double> y_matrix(3, 1, -1); y_matrix.data[y_num-1][0] = 1;
			y.push_back(y_matrix);
			// x mit 4 Merkmalen
			x.push_back(Matrix<double>(line.substr(0, line.size()-2)));
		}
		file_train.close();
	}
	else 
	...
	return vector<vector<Matrix<double>>>{
		vector<Matrix<double>>(x.begin(), x.begin() + split_pos),	// x_train
		vector<Matrix<double>>(y.begin(), y.begin() + split_pos),	// y_train
		vector<Matrix<double>>(x.begin() + split_pos, x.end()),		// x_test
		vector<Matrix<double>>(y.begin() + split_pos, y.end())		// y_tets
	};
}